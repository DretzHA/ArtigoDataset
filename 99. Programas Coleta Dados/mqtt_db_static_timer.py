...
exit_program = False
conn_pool = None
real_x = 0  # Initial value set to 0
real_y = 0
real_z = 0
test_created = False  # New flag to track if the test has been created in the database
test_timeout = 180  # Test duration after receiving the first beacon
...
async def main():
    ...
    # Prompt user for real x, y, z coordinates
    try:
    ...
    client.subscribe("silabs/aoa/angle_and_iq_report/+/ble-pd-B43A31EF7B45")
    client.subscribe("silabs/aoa/position/positioning-LabSC/ble-pd-B43A31EF7B45")
    client.subscribe("silabs/data/periodic_sync_report/+/ble-pd-B43A31EF7B45")
    client.loop_start()

    is_collecting = True
    print("Data collection started immediately.")
    ...
    try:
        ...
        if data_anchors and data_positions:
            ...
            final_df.to_csv(f"Datasets/Calibration/Data/{test_name}_data.csv", index=False)
            print(f"Data saved to {test_name}_data.csv")

        df_timeouts = pd.DataFrame(data_timeouts)
        df_timeouts.to_csv(f"Datasets/Calibration/PeriodicSync/{test_name}_data.csv", index=False)
